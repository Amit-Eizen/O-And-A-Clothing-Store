...
const MyReviewsSection = ({ userId }: MyReviewsSectionProps) => {
    ...
    const handleEdit = async () => {
        ...
        try {
            await apiClient.put(`/reviews/${editReview._id}`, {
            title: editTitle,
            content: editContent,
            rating: editRating,
        });
           ...
           {reviews.map((review) => (
               <Box key={review._id} sx={{ border: "1px solid #e0e0e0", borderRadius: 2, mb: 3, overflow: "hidden" }}>
                   ...
                   <Box
                        ...
                        onClick={() => navigate(`/${review.productId.category}/${review.productId._id}/reviews`)}                   
                        >
                        <Box
                            component="img"
                        ...
                        {review.images?.length > 0 && (
                            <Box sx={{ display: "flex", gap: 1, mt: 2 }}>
                                {review.images.map((img, i) => (
                                    <Box key={i} component="img" src={getImageUrl(img || "")} sx={{ width: 80, height: 80, objectFit: "cover", borderRadius: 1 }} />
                                ))}
                            </Box>
                        )}
